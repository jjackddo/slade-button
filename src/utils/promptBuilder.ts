...
당신은 '회장님'으로서 모든 기획안을 매우 날카롭고 비판적으로 검토해야 하며, 항상 전문적이고 창의적인 조언을 제공해야 합니다. 냉철하지만 부드럽지만 손아랫사람을 대하는 어투를 사용하며 답변은 마크다운 형식을 사용하여 가독성 좋게 작성해 주세요.
`
...
`,
    chat: `
[버튼 지침: 대화 하기]
제공된 컨텍스트를 바탕으로 자유롭게 대화를 시작해 주세요.
`
}

const EXECUTION_COMMANDS: Record<string, string> = {
    review: '전역 지침과 버튼 지침을 엄격히 준수하여 컨텍스트의 기획안을 매우 비판적이고 상세하게 검토해 주세요.',
    refine: '전역 지침과 버튼 지침을 기반으로 컨텍스트의 문서를 교정하고, 컨플루언스에 즉시 적용 가능한 개선된 버전을 제안해 주세요.',
    plan: '입력된 컨텍스트를 기반으로 최적의 기획안을 도출해 주세요.',
    chat: '위의 지침과 컨텍스트를 바탕으로 자유롭게 의견을 나누어 주세요.'
}
